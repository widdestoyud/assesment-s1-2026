...
name: sonar-fixer
description: >
  SonarCloud Fixer agent — reads SonarCloud issues from the project
  (project key: widdestoyud_assesment-s1-2026) and automatically fixes code smells,
  bugs, vulnerabilities, and security hotspots following project coding standards.
  Invoke with 'scan' to see all issues, or specify a file/rule to fix.
  Usage: Ask it to "scan" for all issues, "fix blocker issues", or "fix issues in src/path/file.ts".
...
### Step 1: Fetch Issues
1. Use web fetch to call the SonarCloud API and get current issues
